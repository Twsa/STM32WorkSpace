...
* Description: 	添加模式判断
*/
  if( (cmdPtr->Mode == _RxMode_RC4_EndHeader) || (cmdPtr->Mode == _RxMode_EndHeader))
/* --End-- */
	{
...
* Date:					2017/09/02
* RevisedBy:		Twsa Liu
* Description: 	添加保存数据的选择模式为不带包头包尾，带RC4解密的部分的格式
*/
			
	   else if( (cmdPtr->Mode == _RxMode_RC4_NoEndHeader) || (cmdPtr->Mode == _RxMode_NoEndHeader))
		 {
			 
			 	if(cmdPtr->DataPtr<cmdPtr->BufLen)
				{
					if(cmdPtr->DataPtr==0)  
			    {
				    cmdPtr->Time = cmdPtr->OverTime;        
...
/*************************************************************************************
*  Function Name                    :    CmdCheckErrCount
*  Create Date                      :    2017/08/24
*  Author/Corporation               :    Twsa Liu
*  Description                      :    查看错误次数
...
/*************************************************************************************
*  Function Name                    :    CmdTimeCtrl
*  Create Date                      :    2017/08/24
*  Author/Corporation               :    Twsa Liu
*  Description                      :    超时时间检测上线
...
void CmdRxOvertimeCheck(CMD_BUF* cmdPtr)
{
	if(cmdPtr->DataPtr>0)
	{
		if(cmdPtr->Time==0)
		{
			...
			* Date: 2017/09/02
			* RevisedBy: Twsa Liu
			* Description:对超时的检测处理添加模式判断，模式判断为没有包头包尾时，只表示接受完成
			*/
			if((cmdPtr->Mode == _RxMode_RC4_EndHeader) || (cmdPtr->Mode == _RxMode_EndHeader)) /* --End 1-- */
				 cmdPtr->DataPtr = 0;
			else if( (cmdPtr->Mode == _RxMode_RC4_NoEndHeader) || (cmdPtr->Mode == _RxMode_NoEndHeader))/* --End 2-- */
				cmdPtr->CmdOk =1;
		}
	}
}
...
/*************************************************************************************
*  Function Name                    :    CmdResetOverTime
*  Create Date                      :    2017/08/24
*  Author/Corporation               :    Twsa Liu
*  Description                      :    超时时间定时器重置
...
*--------------------------------Revision History--------------------------------------
*  No         Version         Date        RevisedBy        Item         Description  
*  1          V1.1            2017/09/02  Twsa Liu         None           根据选择的模式进行相应的操作
*************************************************************************************/
#include "arc4.h"
...
void CmdStrToData(CMD_BUF* cmdPtr)
{
	...
	* Date: 2017/09/02
	* RevisedBy :Twsa Liu
	* Description:添加接受数据格式判断
	*/
	if(cmdPtr->Mode == _RxMode_RC4_EndHeader)
	{
	...
	else if(cmdPtr->Mode == _RxMode_RC4_NoEndHeader)
	{
		InfoLength	= cmdPtr->DataPtr; 
	
		if(InfoLength%2==0)//说明长度偶数，正确
		{
			memcpy (InfoUsed,&cmdPtr->Buf[0],InfoLength);         
			
			Str2Hex(InfoUsed ,StrPlain,InfoLength);
			Arc4Decrypt(StrPlain,InfoLength>>1,AllKey);   //公钥解密负载数据
			Str2Hex(StrPlain ,HexPlain,InfoLength>>1);
			memcpy(&cmdPtr->Buf[0],HexPlain,(InfoLength>>2));
//			CmdCheckData(&CommCmdRxBuf);		            //不用检查数据格式
		}
		else //有效长度为奇数，数据错误
		{
...
void CmdStrToData_Simple(CMD_BUF* cmdPtr)
{
	...
	* Description:添加接受数据格式判断
	*/
	if(cmdPtr->Mode == _RxMode_EndHeader)
	{
		InfoLength	= cmdPtr->DataPtr - 2; 
	...
	else if(cmdPtr->Mode == _RxMode_NoEndHeader)
	{
		InfoLength	= cmdPtr->DataPtr; 
		...
		if(InfoLength%2==0)//说明长度偶数，正确
		{
			memcpy (InfoUsed,&cmdPtr->Buf[0],InfoLength);      
			Str2Hex(InfoUsed ,HexPlain,InfoLength);
			memcpy(&cmdPtr->Buf[0],HexPlain,InfoLength);
		}
		else //有效长度为奇数，数据错误
		{
