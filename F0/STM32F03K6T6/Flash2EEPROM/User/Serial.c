...
//  USARTx->BRR  = __USART_BRR(21000000ul, 115200ul);  /* 115200 baud @ 12MHz   */
...
#else
  while (!(USART1->ISR & 0x0080));   //This bit is set by hardware when the content of the USARTx_TDR register has been
                                     // transferred into the shift registerIt is cleared by a write to the USARTx_TDR register
  USART1->TDR = (ch & 0xFF);
#endif
